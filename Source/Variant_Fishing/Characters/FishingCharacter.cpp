...
#include "Variant_Fishing/ActorComponent/FishingFeatures/SubModules
...
FString AFishingCharacter::GetInteractableName_Implementation() const
{
	...
	AFishingPlayerState* PS = TargetController->GetPlayerState<AFishingPlayerState>();
	if (!PS)
	{
		return FString();  
	}
...
EInteractionType AFishingCharacter::GetType_Implementation() const
{
	return EInteractionType::Exchange;
}
...
void AFishingCharacter::ExecuteInteract()
{
	...
	if (IInteractable::Execute_GetType(CurrentOverlapActor) == EInteractionType::Shop)
	{
	...
	else if (IInteractable::Execute_GetType(CurrentOverlapActor) == EInteractionType::Item)
	{
		Server_TryPickup(CurrentOverlapActor); 
		return;
	} else if (IInteractable::Execute_GetType(CurrentOverlapActor) == EInteractionType::Exchange)
	{
		if (AShopCharacter* ShopActor = Cast<AShopCharacter>(CurrentOverlapActor))
		{
			if (IsLocallyControlled())
			{
				if (AFishingPlayerController* PC = Cast<AFishingPlayerController>(GetController()))
				{
					PC->OpenExchangeUI(ShopActor, CoreInventoryComponent);
				}
				return;
			}
		}
...
void AFishingCharacter::ExecuteTab()
{
	
	if (AFishingPlayerController* PC = Cast<AFishingPlayerController>(GetController()))
	{
		PC->ToggleLeaderBoard();
	} 
}
...
void AFishingCharacter::BeginPlay()
{
	...
	AFishingPlayerController* PC = Cast<AFishingPlayerController>(GetController());
	if (PC)
	{
...
void AFishingCharacter::SetupPlayerInputComponent(UInputComponent* PlayerInputComponent)
{
	if (UEnhancedInputComponent* EIC = Cast<UEnhancedInputComponent>(PlayerInputComponent))
	{
		...
		if (TabAction)
		{
			EIC->BindAction(TabAction, ETriggerEvent::Started, this, &AFishingCharacter::ExecuteTab);
		}
	}
	else
	{
